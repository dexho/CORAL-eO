...
class Crop:
    ...
    Crop class
    '''
    def __init__(self,crop_type, filename_full='parameterLists/crop_parameters_forFPSD.xlsx'):
        '''
...
def calc_m_N(t, crop, N_0):
    '''
    Calculates mass of nitrogen taken up as a function of time and ???
    
    k_u is a function of the plant

    '''
    return N_0*np.exp(crop.k_u*t)


def model_m_BN_DDE(y, t, crop, tau, N_0):
    '''
    Calculates m_BN, biomass growth from nitrogen taken up
    
    Assumes RGR is relative to today's growth, while change is based on 
    yesterday's biomass and N
    ...
    m_BN = y(t)
    m_BN_dly = y(t - tau)
    m_N_dly = calc_m_N(t - tau, crop, N_0)
    dm_BNdt = (crop.NP_alpha * m_N_dly / m_BN_dly - crop.NP_beta) * m_BN
    return dm_BNdt

def model_m_BN_ODE(m_BN, t, crop, N_0):
    m_N = calc_m_N(t, crop, N_0)
    dm_BNdt = (crop.NP_alpha - crop.NP_beta * m_BN / m_N) * m_N
    return dm_BNdt

def calc_Y_N_dly(t, crop, tau, N_0, y0):
    '''
    This doesn't get used unless you call it
    It's here because I can't uncouple it from the DDE
    '''
    m_BN_dly = ddeint(model_m_BN_DDE, y0, t, fargs=(crop, tau, N_0)).astype(float)
    m_N_dly = calc_m_N(t - tau, crop, N_0)
    return crop.NP_alpha - crop.NP_beta * m_BN_dly / m_N_dly

def calc_Y_N(t, crop, N_0, y0):
    '''
    This doesn't get used unless you call it
    It's here because I can't uncouple it from the ODE
    '''
    m_BN = integrate.odeint(model_m_BN_ODE, y0, t, args=(crop, N_0)).flatten()
    m_N = calc_m_N(t, crop, N_0)
    return crop.NP_alpha - crop.NP_beta * m_BN / m_N


def model_MEC(t, y, crop, CO2, PPF):
    '''
